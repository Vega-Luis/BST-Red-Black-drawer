...
void node::show(int nivel) {
	for (int i = 0; i < 3 * nivel; i++) std::cout << " ";

	if (this->izq == nullptr && this->der == nullptr) {
		std::cout << "[" << this->v << " <" << this->scolor() << ">" << "]";
	}
	else {
		std::cout << "[" << this->v << " <" << this->scolor() << ">" << std::endl;
		if (this->izq)
			this->izq->show(nivel + 1);
		else {
			for (int i = 0; i < 3 * (nivel + 1); i++) std::cout << " ";
			std::cout << "[ ]";
		}
		std::cout << "\n";
		if (this->der)
...
std::ostream& operator <<(std::ostream& os, link h) {
	h->show(0);
	std::cout << "\n";
	return os;
}
